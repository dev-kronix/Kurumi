...
async function startBot() {
  ...
  if (config.usePairingCode && !conn.authState.creds.registered) {
    ...
    const numero = await new Promise(res =>
      rl.question(chalk.bold.yellowBright('\nDIGITE SEU NÚMERO DE TELEFONE (sem +): '), ans => { rl.close(); res(ans.replace(/\D/g, '')) })
    )
    setTimeout(async () => {
      try {
        ...
        console.error(chalk.bold.bgRed.white(' [ERRO DE PAREAMENTO] '), chalk.bold.redBright(e.message))
        process.exit(1)
      }
    }, 3000)
  }
  ...
  conn.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
    if (connection === 'close') {
      ...
      console.log(chalk.bold.yellowBright(`  ↻ RECONECTANDO EM ${Math.round(delay / 1000)}s...`))
      setTimeout(startBot, delay)
    }

    if (connection === 'open') {
      retryCount = 0
      console.log(chalk.bold.bgGreen.white(`\n ✅ ${config.botName.toUpperCase()} CONECTADA E PRONTA! \n`))
  ...
  conn.ev.on('group-participants.update', async (update) => {
    try {
      ...
      console.log(chalk.bold.blueBright(`[EVENTO] Participantes ${action} em: ${id}`))
      ...
      console.error(chalk.bold.bgRed.white(' [ERRO DE ATUALIZAÇÃO DO GRUPO] '), chalk.bold.redBright(e.stack || e.message))
    }
  })
}
